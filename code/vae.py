...
def loss_function(recon_x, x, mu, logvar, terms=('MSE', 'KLD'), beta=1): 
    '''
    Total Loss =  MSE(recon_x, x) + KLD(mu, logvar)
    ...
    **kwargs:
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, sample_batch in enumerate(dataloader):
            ...
            beta = min((epoch+idx_offset)/100*_beta, _beta)
        ...
        if save_net_freq and save_net_freq > 0:
            if (epoch + idx_offset + 1) % save_net_freq == 0:
...
def extract_latents(net_name, dataset, **kwargs):
    ...
    **kwargs:
    ...
    for batch_idx, sample_batch in enumerate(dataloader):
        ...
        curr_df = pd.DataFrame(sample_batch['info'])
...
if __name__ == "__main__":
    # Prepares the serialized versions of the data
    prepare_serialized(drop_thresh=0.8, fill_empty="mean")
            
    # For running all the different network tests 
    ...
    # For loading the control DTI dataset
    train_control_DTI_dataset, test_control_DTI_dataset = generate_datasets(dataset_type='DTI', non_data_cols=non_data_cols, patient_type='control', percent_split=tt_split, randomize=True, random_seed=0, train_test_split=True, scale_features='robust', fill_empty='mean')
    ...
    trans = transforms.Compose([GaussianNoise(0, 0.1, train_control_DTI_dataset.brain_columns), ToTorchFormat()])
    train_control_DTI_dataset.transform = trans
    test_control_DTI_dataset.transform = trans
    
    # For loading the PTSD DTI dataset
    PTSD_DTI_dataset = generate_datasets(dataset_type='DTI', non_data_cols=non_data_cols, patient_type='PTSD', randomize=True, random_seed=0, scale_features='robust', fill_empty='mean')
    trans = transforms.Compose([GaussianNoise(0, 0.1, PTSD_DTI_dataset.brain_columns), ToTorchFormat()])
    PTSD_DTI_dataset.transform = trans
    
    print(len(PTSD_DTI_dataset.data_columns))
    
    both_DTI_dataset = generate_datasets(dataset_type='DTI', non_data_cols=non_data_cols, patient_type='control_ptsd', randomize=False, scale_features='robust', fill_empty='mean')
    trans = transforms.Compose([GaussianNoise(0, 0.1, both_DTI_dataset.brain_columns), ToTorchFormat()])
    both_DTI_dataset.transform = trans
    
    train_dataset = train_control_DTI_dataset
    test_dataset = test_control_DTI_dataset
    
    net_name = 'control_{}'.format(train_dataset.dataset_type)
    ...
    load_net=False, overwrite=True, overwrite_log=True, l2=0.1, stats_list=stats_list, validation=[test_dataset, PTSD_DTI_dataset])
    
    validation(net_name, test_dataset, overwrite_log=True)
    validation(net_name, PTSD_DTI_dataset)
    
    extract_latents(net_name, both_DTI_dataset, fname_tag='both')
    '''
    ...
    # For loading the control T1 dataset
    train_control_T1_dataset, val_control_T1_dataset, test_control_T1_dataset = generate_datasets(dataset_type='T1_combat', non_data_cols=non_data_cols, patient_type='control', percent_split=tt_split, validation_split=val_split, randomize=True, random_seed=0, train_test_split=True, scale_features='robust', fill_empty='mean')
    # Transforms need to be applied after the dataset is generated since GaussianNoise requires the brain_columns of the dataset
    trans = transforms.Compose([GaussianNoise(0, 0.1, train_control_T1_dataset.brain_columns), ToTorchFormat()])
    train_control_T1_dataset.transform = trans
    val_control_T1_dataset.transform = trans
    ...
    # For loading the PTSD T1 dataset
    PTSD_T1_dataset = generate_datasets(dataset_type='T1_combat', non_data_cols=non_data_cols, patient_type='PTSD', randomize=True, random_seed=0, scale_features='robust', fill_empty='mean')
    # Transforms need to be applied after the dataset is generated since GaussianNoise requires the brain_columns of the dataset
    trans = transforms.Compose([GaussianNoise(0, 0.1, PTSD_T1_dataset.brain_columns), ToTorchFormat()])
    PTSD_T1_dataset.transform = trans
    
    both_T1_dataset = generate_datasets(dataset_type='T1_combat', non_data_cols=non_data_cols, patient_type='control_ptsd', randomize=False, scale_features='robust', fill_empty='mean')
    trans = transforms.Compose([GaussianNoise(0, 0.1, both_T1_dataset.brain_columns), ToTorchFormat()])
    both_T1_dataset.transform = trans
    ...
    for latent_size in latent_sizes:
        for hidden_size in hidden_sizes:
            ...
            train_net(net_name, train_dataset, num_epochs=epochs, until=epochs, hidden_size=hidden_size, latent_size=latent_size, activations=activations,
            load_net=True, overwrite=False, overwrite_log=False, learning_rate=1e-5, l2=0.1, stats_list=stats_list, validation=[val_dataset, PTSD_T1_dataset])
    ...
    train_control_RS_dataset, test_control_RS_dataset = generate_datasets(dataset_type='RS', non_data_cols=non_data_cols, patient_type='control', percent_split=tt_split, randomize=True, random_seed=0, train_test_split=True, scale_features='robust', fill_empty='mean')
    # Transforms need to be applied after the dataset is generated since GaussianNoise requires the brain_columns of the dataset
    ...
    # For loading the PTSD T1 dataset
    PTSD_RS_dataset = generate_datasets(dataset_type='RS', non_data_cols=non_data_cols, patient_type='PTSD', randomize=True, random_seed=0, scale_features='robust', fill_empty='mean')
    # Transforms need to be applied after the dataset is generated since GaussianNoise requires the brain_columns of the dataset
    trans = transforms.Compose([GaussianNoise(0, 0.1, PTSD_RS_dataset.brain_columns), ToTorchFormat()])
    PTSD_RS_dataset.transform = trans
    
    both_RS_dataset = generate_datasets(dataset_type='RS', non_data_cols=non_data_cols, patient_type='control_ptsd', randomize=False, scale_features='robust', fill_empty='mean')
